...
CATALOG-001 (backend substage), 10 сентября 2026. Base `8567578416d25d0ae4dec74e420abb55940eb8a8` / SETTINGS-001 technical CI pass, PR #19. Подготовлен versioned server catalog для одной существующей OpenRouter image capability: immutable model/connection drafts, offline contract proof, publish/disable, credential binding metadata и rotation/revoke. Raw provider secret не принимается: хранится только allowlisted logical `secret_ref`; credential metadata читается отдельно под `secrets.bind`. Endpoint задаётся adapter-кодом, не HTTP payload.

Новая connection после publish остаётся `runtime_state=disabled`; migration `0010_catalog` на этом этапе физически разрешает только `disabled`. Offline proof всегда `network_called=false/live_ready=false`. `JobService` HTTP admission больше не создаёт OpenRouter config из shared env: external quote проходит только через database catalog resolver; полный `IZO_OPENROUTER_*` env без активированного catalog не включает платную генерацию. Worker/test explicit settings сохранены только для исполнения уже принятого snapshot и изолированных provider tests. Live activation/secret resolver/paid probe отсутствуют.

Локально профильные CATALOG/HTTP/migration/SETTINGS tests и соседние Jobs/OpenRouter suites пройдены по отдельным группам; combined длинный run упирался в лимит локального инструмента без assertion failure. PostgreSQL race/rotation/restart acceptance добавлен в существующий Compose gate, но не считается пройденным до exact-head GitHub CI. Canonical OpenAPI подготовлен с сохранением известного pinned FastAPI binary-body representation; окончательную идентичность подтверждает только CI `export_contracts --check`. Machine scope — 20 разрешённых путей при лимите20; runtime resolver вынесен отдельно после architecture size gate, который не ослаблялся. ADMIN UI A-08…A-12 не входит в этот backend substage и остаётся следующим CATALOG UI шагом. **LIVE KEY/CALL/SPEND NO · MERGED/DEPLOYED NO.**
