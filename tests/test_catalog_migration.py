"""CATALOG-002 migration shape on SQLite; PostgreSQL immutability is CI acceptance."""
...
def migration():
    path = ROOT/'apps/api/migrations/versions/0011_catalog.py'
...
def test_catalog_migration_matches_metadata_and_empty_downgrade(tmp_path):
    ...
    m = migration()
    assert m.revision == '0011_catalog' and m.down_revision == '0010_access'
...
def test_populated_catalog_downgrade_refuses_history_loss(tmp_path):
    ...
    with engine.begin() as conn:
        with Operations.context(MigrationContext.configure(conn)):
            m.upgrade()
            conn.execute(sa.insert(t.capability_revisions).values(id=uuid4(), capability_id='x',
                revision=1, content_json='{}', content_hash='0'*64, created_at=1))
...
def test_catalog_migration_is_frozen_and_has_no_application_imports():
    text = (ROOT/'apps/api/migrations/versions/0011_catalog.py').read_text(encoding='utf-8')
    assert 'from izo.' not in text and 'import izo.' not in text
    assert 'runtime_state = \'disabled\'' in text
    assert 'immutable catalog history' in text
