...
def evidence(head='a'*40):
    return {
        'type': 'pr_merge_tree', 'source_head': head, 'verified_pr': 30,
...
def test_current_uses_parent_base_and_verified_working_head_policy():
    ...
    assert 'working_branch=catalog/versioned-provider' in text
    assert 'active_package=CATALOG-002' in text
    assert 'next_package=CATALOG-UI-002' in text
...
def test_pr_merge_tree_evidence_binds_source_head_and_required_workflows():
    ...
    result = validate_pr_evidence(
        ...
        expected_head=head, pr_number=30, foundation_tree='c'*40,
    )
...
def test_pr_evidence_rejects_stale_merge_tree():
    ...
    with pytest.raises(ValueError, match='tested merge tree'):
        validate_pr_evidence(
            pr={'headRefOid': head, 'baseRefOid': 'b'*40, 'state': 'OPEN'}, runs=runs,
            merge_sha='c'*40, merge_commit={'parents': [{'sha': 'b'*40}, {'sha': head}]},
            expected_head=head, pr_number=30, foundation_tree='d'*40,
        )
...
def test_transition_advances_from_verified_working_head_and_checks_dependencies():
    source = plan()
    head = 'a' * 40
    assert dependency_problems(source, 'CATALOG-UI-002', finishing='CATALOG-002') == []
    updated = transition(source, activate='CATALOG-UI-002', next_id=None,
        new_branch='catalog/admin-ui', source_head=head, evidence=evidence(head))
    assert source['active_package'] == 'CATALOG-002'
    assert updated['packages']['CATALOG-002']['status'] == 'technical_pass'
    assert updated['packages']['CATALOG-002']['evidence']['source_head'] == head
    assert updated['active_package'] == 'CATALOG-UI-002'
...
def test_transition_rejects_unready_dependency():
    ...
    with pytest.raises(ValueError, match='ADMIN-001'):
        transition(source, activate='CATALOG-UI-002', next_id=None,
            new_branch='catalog/admin-ui', source_head='a'*40, evidence=evidence())
...
def test_begin_next_rolls_back_branch_and_state_on_write_failure(tmp_path, monkeypatch):
    ...
    def fake_git(*args, root=tmp_path):
        ...
        if args == ('branch','--show-current'): return 'catalog/versioned-provider'
        if args == ('rev-parse','HEAD'): return 'a'*40
        if args[:2] == ('switch','-c'): calls.append('create'); return ''
        if args == ('switch','catalog/versioned-provider'): calls.append('rollback'); return ''
    ...
    with pytest.raises(OSError, match='disk failure'):
        state.begin_next(source, branch='catalog/admin-ui', activate='CATALOG-UI-002',
                         next_id=None, verified_pr=31, root=tmp_path)
...
def test_transition_requires_direct_dependency_on_finishing_package():
    source = plan()
    source["packages"]["CATALOG-UI-002"]["depends_on"] = ["ADMIN-001", "ACCESS-001"]
    with pytest.raises(ValueError, match="next_package must directly depend on the active package"):
        transition(source, activate="CATALOG-UI-002", next_id=None, new_branch="catalog/admin-ui",
...
def test_transition_rejects_completed_or_unrelated_next_package():
    ...
    with pytest.raises(ValueError, match="not eligible from status technical_pass"):
        transition(source, activate="CATALOG-UI-002", next_id="AUTH-001", new_branch="catalog/admin-ui",
                   source_head="a"*40, evidence=evidence())
    with pytest.raises(ValueError, match="must directly depend on activating package CATALOG-UI-002"):
        transition(source, activate="CATALOG-UI-002", next_id="PROFILE-001", new_branch="catalog/admin-ui",
                   source_head="a"*40, evidence=evidence())
