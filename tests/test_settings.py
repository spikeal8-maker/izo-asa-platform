"""SETTINGS-001 lifecycle on real typed Entitlement storage; SQLite is not PG lock proof."""
...
def settings_env(admin_env):
    ...
    with admin.auth.engine.begin() as conn:
        conn.execute(sa.insert(accounts.permissions).values(
            account_id=users[0], permission="plans.write"))
        conn.execute(sa.insert(ent.defaults).values(id=1, version=0))
    return SettingsService(admin.auth), users, sessions


def publish(service, session, expected, policy=None, operation=None):
    ...
    return service.publish(session.bearer, session.view.csrf_token, data), data


def test_read_requires_plans_write(settings_env):
    service, _, sessions = settings_env
    with pytest.raises(Exception) as exc:
        service.view(sessions[1].bearer)
    assert getattr(exc.value, "code", None) == "forbidden"


def test_empty_default_is_explicit_deny_with_descriptors(settings_env):
    service, _, sessions = settings_env
    view = service.view(sessions[0].bearer)
    ...
    assert {item.id for item in view.descriptors} >= {"S-10", "S-11", "S-12", "S-13", "S-14", "S-15", "S-17", "S-22"}


def test_preview_is_read_only_diff_and_impact(settings_env):
    service, _, sessions = settings_env
    preview = service.preview(sessions[0].bearer,
...
def test_req_blocks_enabled_generation(settings_env, change, missing):
    service, _, sessions = settings_env
    policy = valid_policy(**change)
    preview = service.preview(sessions[0].bearer, PreviewInput(expected_revision=0, policy=policy))
    ...
    with pytest.raises(SettingsError, match="required_setting_missing") as exc:
        publish(service, sessions[0], 0, policy)
    assert missing in exc.value.fields


def test_deny_all_policy_can_be_published_safely(settings_env):
    service, _, sessions = settings_env
    receipt, _ = publish(service, sessions[0], 0, PlanPolicy())
    assert receipt.default_version == 1
    assert not service.view(sessions[0].bearer).generation_enabled


def test_publish_exact_retry_is_one_revision_and_one_default_change(settings_env):
    service, _, sessions = settings_env
    operation = uuid4()
    first, data = publish(service, sessions[0], 0, operation=operation)
    second = service.publish(sessions[0].bearer, sessions[0].view.csrf_token, data)
...
def test_expected_revision_conflict_does_not_create_revision(settings_env):
    service, _, sessions = settings_env
    publish(service, sessions[0], 0)
    with pytest.raises(SettingsError, match="revision_conflict"):
        publish(service, sessions[0], 0, valid_policy(active_jobs=3))
    with service.auth.engine.connect() as conn:
        assert conn.execute(sa.select(sa.func.count()).select_from(ent.revisions)).scalar_one() == 1
...
def test_rollback_creates_new_revision_without_rewriting_history(settings_env):
    service, _, sessions = settings_env
    first, _ = publish(service, sessions[0], 0, valid_policy(active_jobs=2))
    second, _ = publish(service, sessions[0], 1, valid_policy(active_jobs=5))
    rolled = service.rollback(sessions[0].bearer, sessions[0].view.csrf_token,
        RollbackInput(operation_id=uuid4(), expected_revision=2,
            target_revision=first.revision, reason="restore reviewed limits"))
    current = service.view(sessions[0].bearer)
    history = service.history(sessions[0].bearer)
