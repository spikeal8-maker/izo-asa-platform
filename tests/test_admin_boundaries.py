...
def test_admin_never_writes_wallet_sql():
    ...
    service=(ROOT/'apps/api/izo/admin/service.py').read_text()
...
def test_accounts_facade_does_not_import_admin_or_ledger():
    tree=ast.parse((ROOT/'apps/api/izo/accounts/admin_access.py').read_text())
...
def test_admin_http_uses_existing_body_and_origin_guards():
    auth=(ROOT/'apps/api/izo/accounts/routes.py').read_text()
    http=(ROOT/'apps/api/izo/accounts/http_security.py').read_text()
    admin=(ROOT/'apps/api/izo/admin/routes.py').read_text()
...
def test_real_browser_and_restart_checks_not_mock_only():
    ...
    script=(ROOT/'apps/web/acceptance/admin-live.mjs').read_text()
...
def test_ui_does_not_fake_admin_or_store_password():
    ...
    shell=(ROOT/'apps/web/src/shell/App.tsx').read_text()
    assert 'Демо-пользователь 01' not in shell
    form=(ROOT/'apps/web/src/features/admin/GrantForm.tsx').read_text()
