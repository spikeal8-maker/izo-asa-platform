"""Real PostgreSQL + HTTP acceptance for public open signup and durable sessions.
...
Closed invite mode is separately covered by unit/boundary tests; public acceptance uses open registration.
"""
import json
import os
import secrets
...
class Client:
    ...
    def call(self, method, path, data=None, headers=None):
        ...
        if self.raw: sent["Cookie"] = f"{self.name}={self.raw}"
        if self.csrf: sent["X-CSRF-Token"] = self.csrf
        ...
        with response:
            raw = response.read(65536); decoded = json.loads(raw) if raw else None
            cookie = SimpleCookie(response.headers.get("Set-Cookie", ""))
            if self.name in cookie: self.raw = cookie[self.name].value
...
def before(engine, service):
    ...
    for client, payload in [(alice, a_data), (bob, b_data)]:
        ...
        check(status == 201, "open_register")
    ...
    status, listed = alice.call("GET", "/sessions"); old_session = listed["sessions"][0]["id"]
    ...
    # Open registration still serializes one identity: concurrent same-email requests produce one account.
    ...
    # Two different identities may register concurrently in public mode.
    variants = [data(), data()]
    with ThreadPoolExecutor(max_workers=2) as pool:
        results = list(pool.map(lambda payload: Client(cookie).call("POST", "/register", payload), variants))
    check([status for status, _ in results].count(201) == 2, "distinct_open_signup")
    accounts.extend(output["account"]["id"] for status, output in results if status == 201)

    rate_id, window = "f" + uuid4().hex + "0" * 31, service.now()
    def counter(_):
        with engine.begin() as conn: return repo.consume_rate(conn, rate_id, window, 5)
    with ThreadPoolExecutor(max_workers=6) as pool:
        counts = list(pool.map(counter, range(12)))
    check(sum(count <= 5 for count in counts) == 5, "atomic_rate_limit")

    cap_data = data(); cap_client = Client(cookie)
    status, output = cap_client.call("POST", "/register", cap_data)
    check(status == 201, "cap_account"); accounts.append(output["account"]["id"])
    ...
    def login(_):
        try:
            ...
            return 200
        except AuthError as exc: return exc.status
    with ThreadPoolExecutor(max_workers=2) as pool:
    ...
    print("AUTH_BEFORE_OK: open signup, HTTP, PostgreSQL races, revocation, scopes and hashing", file=sys.stderr)
    print(json.dumps({"schema": 2, "cookie": cookie, "active": alice.raw, "revoked": old_cookie,
                      "account": accounts[0], "accounts": accounts, "rate_id": rate_id}))


def after(engine):
    fixture = json.loads(sys.stdin.read(16384)); check(fixture["schema"] == 2, "fixture_version")
    ...
    print("AUTH_AFTER_OK: open account/session/revocation persisted across Compose down/up; test accounts removed")
...
def main():
    ...
    policy = AuthSettings(); policy.require_configured()
    if policy.registration != "open": raise SystemExit("Public AUTH acceptance requires open registration")
    engine = repo.create_auth_engine(config)
    try:
        phase = sys.argv[1] if len(sys.argv) == 2 else ""
        if phase == "before": before(engine, AuthService(engine, policy))
        elif phase == "after": after(engine)
