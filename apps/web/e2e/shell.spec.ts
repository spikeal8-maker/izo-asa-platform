...
test('chat-first home is the primary product surface', async ({ page }) => {
  ...
  await expect(page.getByRole('navigation', { name: 'Режимы ИЗО АСА' }).getByRole('link', { name: 'Чат', exact: true })).toHaveAttribute('aria-current', 'page')
...
test('mobile keeps chat modes on top and short product navigation below', async ({ page }, info) => {
  ...
  const directions = page.getByRole('navigation', { name: 'Режимы ИЗО АСА' })
