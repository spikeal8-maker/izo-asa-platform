...
test('chat-first home is the primary product surface', async ({ page }) => {
  ...
  await expect(page.getByRole('navigation', { name: 'Творческие инструменты ИЗО АСА' }).getByRole('link', { name: 'Чат', exact: true })).toHaveAttribute('aria-current', 'page')
...
test('mobile keeps chat modes on top and short product navigation below', async ({ page }, info) => {
  test.skip(!info.project.name.startsWith('phone'))
  await page.goto('/')
  await expect(page.getByRole('button', { name: 'Меню', exact: true })).toBeVisible()
  const directions = page.getByRole('navigation', { name: 'Творческие инструменты ИЗО АСА' })
  await expect(directions.getByRole('link')).toHaveCount(5)
  await expect(directions.getByRole('link', { name: 'Изображение', exact: true })).toBeVisible()
  ...
  await expect(primary.getByRole('link', { name: 'Чат', exact: true })).toBeVisible()
  expect(await page.evaluate(() => document.documentElement.scrollWidth <= innerWidth)).toBe(true)
})
