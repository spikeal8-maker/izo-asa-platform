...
Сейчас разрешён только test.image.v1: диагностическое изображение, НЕ AI. Только этот
чистый тестовый адаптер повторяется после истёкшей незаписанной попытки. Нельзя
подключить внешний/локальный AI к этой ветке восстановления без собственного
идемпотентного/reconciliation-контракта и тестов. HTTP worker-команд нет.
