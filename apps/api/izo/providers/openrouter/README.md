...
через общий media codec/S3. Prompt — данные, не shell/workflow. Response body и ключи не логируются.
...
Live probe отсутствует в CI и не должен добавляться с реальным ключом. Актуальный контракт API
сверять с официальной документацией OpenRouter при каждом изменении адаптера.
