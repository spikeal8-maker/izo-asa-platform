...
def generate(snapshot: dict, prompt: str, api_key: str, *, timeout: int,
             opener=safe_urlopen) -> ProviderImage:
    body = json.dumps({
    ...
    try:
        ...
        # A server reply still follows a network dispatch. Keep the reservation;
        # do not infer billing/non-billing from text in a remote error body.
        raise ProviderError("provider_rejected", request_may_have_completed=True) from None
