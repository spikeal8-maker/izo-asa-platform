...
def execute_one(runner, settings, secret):
    ...
    try:
        ...
        api_key = secret.require()  # No network has occurred yet.
    except Exception:
        try:
    ...
    try:
        ...
        # Every adapter error occurs after dispatch; no automatic paid retry.
        try:
    ...
    try:
        ...
        # At this point a paid result may already exist; preserve the reservation.
        try:
            runner.provider_uncertain(claim, "provider_result_storage_uncertain")
        except JobError:
            # If seal already advanced state, existing S3 reconciliation owns it.
            try:
